$(function(){
	$('#btn-fieldset-admins_pages,#btn-fieldset-pages').click(function(){
		var text = this.id;
	    var id = text.replace('btn-', '');
	    $('#'+id).toggle();
	});
	
	$('.p_collapse').click(function(){
    ...
    function reqs(data){
        Ext.Ajax.request({
			url: intelli.config.admin_url + '/manage/permissions.json' + window.location.search,
			method: 'post',
			params: data,
            success: function(d){
    ...
    var first = true;
	$('#input-all--admin_login--read').change(function(){
		if(this.value == 1)
		{
			$('#div-'+this.id).show();
		}
	...
	$('.light input[type="hidden"]').change(function(){
	...
	$('.p_links div').mousedown(function(){
		$(this).addClass('click');
	}).mouseup(function(){
	...
	$('.p_links .save').click(function(){
		$(this).hide();
		save_perms($($(this).parents('.p_hover')));
		return false;
	}).hide();
	$('.p_links .disallow').click(function(){
		...
		divs.each(function(){
			...
			$('#input-'+id).val(0);
			if($('#box-'+id).is(':checked')) $('#box-'+id).removeAttr('checked').change();
		});
		parent.addClass('modified');
		return false;
	});
	$('.p_links .allow').click(function(){
		var parent = $($(this).parents('.p_hover'));
		var divs = parent.find('.'+parent.attr('id').replace('p_cont-', 'object-')+' input[id^="default-"]');
		divs.each(function(){
			var id = this.id.replace('default-', '');
			$('#input-'+id).val(1);
			if(!$('#box-'+id).is(':checked')) $('#box-'+id).attr('checked', 'checked').change();
		});
		parent.addClass('modified');
		return false;
	});
	$('.p_links .set_default').click(function(){
		var parent = $($(this).parents('.p_hover'));
		var divs = parent.find('.'+parent.attr('id').replace('p_cont-', 'object-')+' input[id^="default-"]');
		divs.each(function(){
			var id = this.id.replace('default-', '');
			$('#input-'+id).val(this.value).addClass('default');
			$('#box-'+id).attr('checked', this.value == 1 ? 'checked' : '').change();
		});
