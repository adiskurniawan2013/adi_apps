...
$(document).ready(function() {
	function submit_form(querystr){
		...
		$.ajax({
			url		:	"<?php echo base_url('customer/submit/'.@$person_id) ?>",
			type	:	"POST",
			dataType:	"json",
			data	:	querystr,
			...
			$("#loading div").fadeOut(300);
				if(data.server_validation) {
					...
					if(!data.is_valid) {
						...
						load_content('customer','#tab_loader');
					}
				}
			}//END SUCCESS
		});
	...
	<div id="form_loader">
		<form action="<?php echo base_url('customer/submit/'.@$person_id) ?>" method="post" onsubmit="return false" id="form">
				    ...
				    <div id="contentBoxTwoEdit" >
						<table class="editTable">
							...
							<tr>
								...
								<td class="cellRightEditTable">
									<input type="text" id="taxable" name="taxable" value="<?php echo @$taxable ?>">
								</td>
							</tr>
							<tr>
								<td class="cellLeftEditTable">
			...
			<div>
				...
				&nbsp;&nbsp;
				<input name="cancel" type="button" id="cancel" value="Cancel" onclick="javascript:load_content('customer','#tab_loader')"/>
			</div> 
